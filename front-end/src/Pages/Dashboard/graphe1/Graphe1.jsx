...
/* 📌 Convertir dynamiquement : Wh → kWh → MWh → GWh */
function formatUnit(valueWh) {
  const abs = Math.abs(valueWh);

  if (abs >= 1e9) return (valueWh / 1e9).toFixed(2) + " GWh";
  if (abs >= 1e6) return (valueWh / 1e6).toFixed(2) + " MWh";
  if (abs >= 1e3) return (valueWh / 1e3).toFixed(2) + " kWh";
    ...
    const fetchAndAggregate = async () => {
      ...
      try {
        ...
        consos.forEach(c => {
          ...
          const valWh = Number(c.consommation_Wh) || 0;

          if (!agg[y]) agg[y] = new Array(12).fill(0);

          agg[y][m] += valWh; // On garde Wh → conversion affichage seulement
        });
      ...
      <header className="pc-head">
        <div className="pc-title">
          ...
          <div>
            ...
            <p>Consommation mensuelle en énergie</p>
          </div>
        </div>

        <div className="pc-actions">
          <div className="pc-select" role="button">
            <RiCalendar2Line/>
      ...
      <section className="pc-grid">
        <div className="pc-card glass">
          ...
          >
            ...
            <path d={pathLine} className="pc-line"/>

            {/* Mois */}
            {MONTHS.map((m,i)=>(
              <text key={m} x={scaleX(i)} y={view.h-view.padB+18} className="pc-x" textAnchor="middle">{m}</text>
            ))}
            ...
            {hover && (
              <>
                ...
                <g transform={`translate(${hover.x+10}, ${hover.y-28})`} className="pc-tip">
                  <rect rx="8" ry="8" width="150" height="46"/>
                  <text x="10" y="18" className="pc-tip-title">{MONTHS[hover.i]} {year}</text>
                  <text x="10" y="34" className="pc-tip-value">{formatUnit(hover.val)}</text>
                </g>
              </>
            )}
        ...
        <aside className="pc-side">
          <div className="pc-mini glass">
            ...
            <strong className="pc-mini-value">{formatUnit(sum)}</strong>
          </div>

          <div className="pc-mini glass">
            <span className="pc-mini-title">Pic mensuel</span>
            <strong className="pc-mini-value">{formatUnit(max)}</strong>
          </div>

          <div className="pc-mini glass">
            <span className="pc-mini-title">Moyenne</span>
            <strong className="pc-mini-value">{formatUnit(avg)}</strong>
          </div>
        </aside>
      </section>
...
function useChart(series){
  ...
  const scaleX=i=>padL+(innerW/11)*i;
